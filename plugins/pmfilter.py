    ...
    try:
        ...
        if pm_search:
            ...
            await message.reply_text(
             text=f"<b>🙋 ʜᴇʏ {user} 😍 ,\n\n𝒀𝒐𝒖 𝒄𝒂𝒏 𝒔𝒆𝒂𝒓𝒄𝒉 𝒇𝒐𝒓 𝒎𝒐𝒗𝒊𝒆𝒔 𝒐𝒏𝒍𝒚 𝒐𝒏 𝒐𝒖𝒓 𝑴𝒐𝒗𝒊𝒆 𝑮𝒓𝒐𝒖𝒑. 𝒀𝒐𝒖 𝒂𝒓𝒆 𝒏𝒐𝒕 𝒂𝒍𝒍𝒐𝒘𝒆𝒅 𝒕𝒐 𝒔𝒆𝒂𝒓𝒄𝒉 𝒇𝒐𝒓 𝒎𝒐𝒗𝒊𝒆𝒔 𝒐𝒏 𝑫𝒊𝒓𝒆𝒄𝒕 𝑩𝒐𝒕. 𝑷𝒍𝒆𝒂𝒔𝒆 𝒋𝒐𝒊𝒏 𝒐𝒖𝒓 𝒎𝒐𝒗𝒊𝒆 𝒈𝒓𝒐𝒖𝒑 𝒃𝒚 𝒄𝒍𝒊𝒄𝒌𝒊𝒏𝒈 𝒐𝒏 𝒕𝒉𝒆  𝑹𝑬𝑸𝑼𝑬𝑺𝑻 𝑯𝑬𝑹𝑬 𝒃𝒖𝒕𝒕𝒐𝒏 𝒈𝒊𝒗𝒆𝒏 𝒃𝒆𝒍𝒐𝒘 𝒂𝒏𝒅 𝒔𝒆𝒂𝒓𝒄𝒉 𝒚𝒐𝒖𝒓 𝒇𝒂𝒗𝒐𝒓𝒊𝒕𝒆 𝒎𝒐𝒗𝒊𝒆 𝒕𝒉𝒆𝒓𝒆 👇\n\n<blockquote> మీరు మా Movie Group లో మాత్రమే సినిమాలను సెర్చ్ చేయగలరు. Direct Bot లో సినిమాలను సెర్చ్ చేయడానికి అనుమతి లేదు.\n"
             "దయచేసి దిగువనున్న REQUEST HERE బటన్‌ను క్లిక్ చేసి మా Movie Group లో చేరి మీ ఇష్టమైన సినిమాను అక్కడ సెర్చ్ చేయండి. 👇 </blockquote></b>",   
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("🌹 ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ 🌹", url=f"https://t.me/Telugu_Movies_999")
            ]
        )
        btn.insert(0, 
            [ 
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("🦸🏻‍♀️ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="ɴᴇxᴛ ⋟🧜‍♀️",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("🦸🏻‍♀️ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="ɴᴇxᴛ ⋟🧜‍♀️",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn = [[
                ...
                InlineKeyboardButton("😗 Not Available In The Telugu 😗", callback_data=f"Not_Available_In_The_Telugu#{from_user}")
             ]]
        btn2 = [[
    ...
    elif query.data.startswith("Not_Available_In_The_Telugu"):
        ident, from_user = query.data.split("#")
        btn = [[
                InlineKeyboardButton("😗 Not Available In The Telugu 😗", callback_data=f"unalert#{from_user}")
              ]]
        btn2 = [[
                 InlineKeyboardButton('ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ', url=link.invite_link),
                 InlineKeyboardButton("ᴠɪᴇᴡ ꜱᴛᴀᴛᴜꜱ", url=f"{query.message.link}")
       ...
       #@Deendayal_dhakad
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton(' Mᴀʀᴠᴇʟ & Dᴄ 🎆', url=f'https://t.me/vs_Auto_Filter_bot'),
                    ...
                    InlineKeyboardButton('🎀 Aɴɪᴍᴇs Mᴀɪɴ Cʜᴀɴɴᴇʟ 🎀', url=f'https://t.me/anime_Movies_and_Series_Telugu')
                ],[
